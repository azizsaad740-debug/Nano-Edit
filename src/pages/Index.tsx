...
export const IndexPage: React.FC<IndexPageProps> = ({ initialImage }) => {
  ...
  // Removed redundant useResizeObserver and useZoomFit calls here, as they are handled below or within the hook logic.
  // The original code had two calls to useResizeObserver and useZoomFit, which is likely incorrect usage.
  // Keeping the logic that was intended to be used for layout calculation:
  const [workspaceSize, setWorkspaceSize] = React.useState({ width: 0, height: 0 });
  const workspaceResizeObserver = useResizeObserver({
    ref: workspaceRef,
    callback: (entry) => {
      setWorkspaceSize({
  ...
  useZoomFit({
    workspaceRef, imgRef, dimensions, zoom, setZoom,
    handleFitScreen,
  });
