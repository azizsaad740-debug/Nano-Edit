...
export const useEditorState = () => {
  ...
  const handleFramePropertyChange = useCallback((key: 'width' | 'color', value: number | string) => {
    ...
    updateCurrentState({ frame: new<dyad-problem-report summary="2 problems">
<problem file="src/hooks/useEditorState.ts" line="1505" column="5" code="1117">An object literal cannot have multiple properties with the same name.</problem>
<problem file="src/hooks/useEditorState.ts" line="1506" column="5" code="1117">An object literal cannot have multiple properties with the same name.</problem>
</dyad-problem-report><think>**Reviewing Duplicate Properties**

I'm currently focused on the `useEditorState.ts` hook. I've pinpointed duplicate property names, namely `groupLayers` and `toggleGroupExpanded`, as the source of two TypeScript errors. My next step involves inspecting the code to understand why this duplication exists and how best to resolve it, perhaps through restructuring or renaming.


**Refining the Return Object**

I've determined the source of the duplicate property errors in `useEditorState.ts`. The properties `groupLayers` and `toggleGroupExpanded` were accidentally repeated within the returned object. My approach now is to remove the redundant declarations near the end of the return statement, keeping the initial destructuring from `useLayers`. This should rectify the TypeScript errors and maintain the intended functionality of the hook.


**Removing Redundancies Effectively**

I have carefully reviewed the `useEditorState.ts` hook. I've confirmed that the source of the TypeScript errors is indeed the repeated declaration of `groupLayers` and `toggleGroupExpanded`. The fix is straightforward: I'll remove the redundant declarations near the end of the returned object, as the initial destructuring from `useLayers` provides the necessary values. This ensures that the hook functions correctly and eliminates the duplicate property errors.


**Confirming Code Modification**

I've examined the code snippet and confirmed the solution. Removing the redundant lines `groupLayers` and `toggleGroupExpanded` will resolve the TypeScript errors in `useEditorState.ts`. I am confident that the proposed change will not affect functionality.


</think>The compile-time errors are due to duplicate property names (`groupLayers` and `toggleGroupExpanded`) in the object returned by `useEditorState`. I will remove the redundant entries.

<dyad-write path="src/hooks/useEditorState.ts" description="Removing duplicate properties (groupLayers and toggleGroupExpanded) from the hook return object.">
import React, { useState, useRef, useCallback } from "react";
import { type Crop } from "react-image-crop";
import { useHotkeys } from "react-hotkeys-hook";
import { showSuccess, showError, showLoading, dismissToast } from "@/utils/toast";
